...
def save_vector_store(text_chunks, index_name="faiss_index"):
...
def get_conversational_chain():
    prompt_template = """
    Answer like OpenAI's ChatGPT or Google's Gemini or Bard would. Be detailed and explanative. 
    If you are asked you provide the answer in a certain format, please do
...
st.sidebar.markdown('<div class ="sidebar-text_tag">Building Sustainable AI</div>', unsafe_allow_html=True)
        ...
        if uploaded_file is not None:
            ...
            if st.button("Analyze Image"):
                with st.spinner("Processing..."):
                    ...
                    text_chunks = get_text_chunks(image_text)
                    save_vector_store(text_chunks)
                    #st.subheader("Extracted Text from Image:")
                    #st.write(image_text)
                    st.success("Image processed and text saved.")
...
st.sidebar.markdown("""
    <div style="text-align: center; padding-top: 10px;">
        <hr>
        <p style="color:gray;font-size:12px;">All Copyrights Reserved Atharva Shitut @2024</p>
    </div>
""", unsafe_allow_html=True)